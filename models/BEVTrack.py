...
class BEVTrack(BaseModel):
    ...
    def __init__(self,
                 ...
                 fuser=None,
                 head=None,
                 cfg=None, use_dmw=False, dmw=None, use_tiw=False, tiw=None):
        super().__init__()
        self.config = cfg
        ...
        # === DMW/TIW init ===
        self.use_dmw = use_dmw
        self.use_tiw = use_tiw
        self.dmw = MODELS.build(dmw) if dmw is not None else None
        self.tiw = MODELS.build(tiw) if tiw is not None else None
    ...
    def get_feats(self, inputs):
        ...
        # === DMW forward ===
        if self.use_dmw and (self.dmw is not None):
            prev_map, this_map = stack_feats.chunk(2, dim=0)
            f_prev = prev_map.mean(dim=(2,3))
            f_this = this_map.mean(dim=(2,3))
            delta_base = results['coors']
            delta_dmw, f_enhanced, w_dmw = self.dmw(f_prev, f_this, delta_base)
            results['coors_base'] = delta_base
            results['coors'] = delta_dmw
            results['feat_enhanced'] = f_enhanced
            results['w_dmw'] = w_dmw
    ...
    def predict(self, inputs):
        ...
        hist = deque(maxlen=self.tiw.L if (self.tiw is not None) else 5)
        ious = []
        distances = []
        results_bbs = []
